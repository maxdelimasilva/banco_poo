...
class ContaSimples:
    ...
    def depositar(self, valor):
        if valor > 0:
            self.saldo += valor
            self.historico.append((f"Depósito", valor, datetime.now()))
    ...
    def sacar(self, valor):
        if valor > 0 and valor <= self.saldo:
            self.saldo -= valor
            self.historico.append((f"Saque", valor, datetime.now()))
    ...
    def extrato(self):
        ...
        for tipo, valor, data in self.historico:
            print(f"{data.strftime('%d/%m/%Y %H:%M:%S')} - {tipo}: R${valor:.2f}")
        print(f"Saldo atual: R${self.saldo:.2f}\n")

    def __str__(self):
        return f"Conta {self.numero} - Titular: {self.cliente.nome}"
